...
static CGFloat HJScreenWidth;
static CGFloat HJScreenScale;

@implementation HJPhotoKitManager

#pragma mark - 配置

+ (instancetype)manager {
    static HJPhotoKitManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[self alloc] init];
        
        //初始化大小
        HJScreenWidth = [UIScreen mainScreen].bounds.size.width;
        HJScreenScale = 2.0;
        if (HJScreenWidth > 700) {
            HJScreenScale = 1.5;
        }
...
- (void)getPhotosBytesWithArray:(NSArray <HJAssetModel *>*)photos completion:(void (^)(NSString *totalBytes))completion {
    if (photos.count < 1) {
        completion([self getBytesFromDataLength:0]);
        return;
    }
    __block NSInteger dataLength = 0;
    __block NSInteger assetCount = 0;
    for (NSInteger i = 0; i < photos.count; i++) {
        HJAssetModel *model = photos[i];
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if (model.type != HJAssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            ...
            if (model.type != HJAssetModelMediaTypeVideo) dataLength += (NSInteger)representation.size;
#pragma clang diagnostic pop
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes);
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        CGFloat pixelWidth = photoWidth * HJScreenScale;
...
- (HJAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    HJAlbumModel *model = [[HJAlbumModel alloc] init];
    model.result = result;
    model.name = name;
